...
def split_into_chunks(text: str, tokenizer, max_length: int) -> List[str]:
    ...
    # print(f"Original token length: {len(tokens)}")  # Debugging: print original token length
    
    # Split tokens into chunks of max_length - 2 to account for special tokens
    chunks = []
    for i in range(0, len(tokens), max_length - 2):
        ...
        if len(chunk_tokens) <= max_length:
            ...
            # Further split if the chunk with special tokens exceeds max_length
    ...
    # Print lengths of chunks for debugging
    # for chunk_text in chunks:
    #     chunk_tokens = tokenizer.encode(chunk_text, add_special_tokens=True)
    #     print(f"Chunk token length with special tokens: {len(chunk_tokens)}")  # Debugging: print chunk token length with special tokens
...
def get_embeddings(input: List[str], tokenizer: type[Any], model: type[AutoModel], max_length: int) -> torch.Tensor:
    ...
    for document in input:
        ...
        chunk_embeddings = []
        for chunk in chunks:
            # print(f"Processing chunk: {chunk[:50]}... (length: {len(chunk)})")
...
def process_embeddings(input: List[str], model_name: str, batch_size: int = 32, save_path: str = "") -> torch.Tensor:
...
if __name__ == "__main__":
    ...
    nomic_embeddings = process_embeddings(strings, "nomic-embed-text-v1.5")
    print("Nomic Model Embeddings:")
    print(nomic_embeddings.size())
    print(nomic_embeddings)
    
    st_embeddings = process_embeddings(strings,  "all-MiniLM-L6-v2")
    print("Sentence Transformer (all-mini) Embeddings:")
    print(st_embeddings.size())
    print(st_embeddings)

    st_embeddings = process_embeddings(strings,  "all-mpnet-base-v2")
    print("Sentence Transformer (all-mpnet) Embeddings:")
    print(st_embeddings.size())
    print(st_embeddings)

    bert_embeddings = process_embeddings(strings, "bert_uncased")
